...
c---------------------------------------------------------------------
      ...
      include 'mpinpb.h'
c      integer status(MPI_STATUS_SIZE), request, ierr

      include 'npbparams.h'
      !include 'mpp/shmem.fh'
c---------------------------------------------------------------------
c  num_procs must be a power of 2, and num_procs=num_proc_cols*num_proc_rows.
c  num_proc_rows and num_proc_cols are to be found in npbparams.h.
...
CC      >           rcond=1.0d-1 )
     ...
     >                        	 w(na/num_proc_rows+2)
      ...
      double precision ,allocatable :: norm_temp1(:)[:]
      double precision , allocatable :: norm_temp2(:)[:] 
      double precision  mflops
      double precision, allocatable  :: t(:)[:]
      double precision, save :: tmax
      double precision, save :: remote_time
      external           timer_read
...
c     allocations ***************
        allocate (norm_temp1(2)[*])
        allocate (norm_temp2(2)[*])
        allocate (t(1)[*])

c      integer, dimension(SHMEM_BCAST_SYNC_SIZE), save :: psync
c      double precision, dimension(SHMEM_REDUCE_MIN_WRKDATA_SIZE), 
c     >  save :: pwrk
...
c            call mpi_wait( request, status, ierr )
            ...
            norm_temp2(1:2)[other] = norm_temp1(1:2)
            sync all
...
c            call mpi_wait( request, status, ierr )
            ...
            norm_temp2(1:2)[other] = norm_temp1(1:2)
            sync all 
            norm_temp1(1:2) = norm_temp1(1:2) + norm_temp2(1:2)
         enddo
...
c     >                 mpi_comm_world,
c     >                 ierr )
c     call shmem_real8_max_to_all(tmax,t,1,0,0,nprocs,pwrk,psync)

      !call caf_real8_max_to_all(tmax,t(1))
      tmax=0
      sync all
...
c    integer status(MPI_STATUS_SIZE ), request
     ...
     >                   w(*)                ! used as work temporary
      ...
      integer   i, j, k, n, ierr, other
      integer   cgit, cgitmax

      double precision ::sum, rho0, beta, rnorm
...
c            call mpi_wait( request, status, ierr )
     ...
     >      !                      reduce_exch_proc(i))
            k =  reduce_exch_proc(i)
            j =  reduce_send_lengths(i)

            q(reduce_recv_starts(i):reduce_recv_starts(i)+j-1)[k] = 
     >              w(reduce_send_starts(i):reduce_send_starts(i)+j-1)
            sync all 
...
c     >                      ierr )
c            call mpi_wait( request, status, ierr )
c     >       call shmem_double_put(q,
c     >                            w(send_start),
c     >                            send_len,
c     >                            exch_proc)
            j = send_len
            q(1:j)[exch_proc] =  w(send_start:send_start+j-1)
            sync all
         else
...
c     >                      ierr )
c            call mpi_wait( request, status, ierr )
c            call shmem_double_put(rho,
c     >                            sum,
c     >                            1,
c     >                            reduce_exch_proc(i))
            rho(1)[reduce_exch_proc(i)] = sum
            sync all
...
c     >                   ierr )
c         call mpi_wait( request, status, ierr )
c           call shmem_double_put( r(reduce_recv_starts(i)),
c     >                            w(reduce_send_starts(i)),
c     >                            reduce_send_lengths(i),
c     >                            reduce_exch_proc(i))
          j=reduce_recv_starts(i)
          k= reduce_send_starts(i)
          n = reduce_send_lengths(i) 
          r(j:j+n-1)[reduce_exch_proc(i)] = w(k:k+n-1)
