...
c     call shmem_real8_max_to_all(tmax,t,1,0,0,nprocs,pwrk,psync)
      ...
      call caf_real8_max_to_all(tmax,t)
      if( me .eq. root )then
         write(*,100)
...
c---------------------------------------------------------------------
     ...
     >                   w(*)                ! used as work temporary
      ...
      double   allocatable :: rho[:] 
      double allocatable :: d [:]
...
c            call mpi_wait( request, status, ierr )
     ...
     >      !                      reduce_exch_proc(i))
            img =  reduce_exch_proc(i)
            N =  reduce_send_lengths(i)
            q(reduce_recv_starts(i))(1:N)[img] = 
     >              w(reduce_send_starts(i))(1:N)
            sync all 
